...
### Ad blocker (hand-rolled domain matching + cosmetic CSS)

- **Network blocking** (`setupAdBlocker` → `session.defaultSession.webRequest.onBeforeRequest` with a `http(s)://*/*` URL filter): every request URL is parsed, and the hostname is matched against the in-memory `adDomains` Set by walking parent domains (`cdn.ads.example.com` → `ads.example.com` → `example.com`). Match → block.
- **Domain list**: `adblocker/lists/default.txt` — a curated ~300-domain starter covering Google, Amazon (incl. `dl.amazon.in`, the ad-creative CDN that slipped through v1.2.0), Meta, Criteo, Taboola, AppNexus, Rubicon, OpenX, Smart, Yahoo, Hotjar, FullStory, Ezoic, Mediavine, etc. On startup, main prefers `userData/adblocker-list.txt` (saved by an Update) and falls back to the bundled list. `loadAdListFromText` accepts both bare domains and hosts format.
- **Cosmetic filtering** (`setupCosmeticFiltering` → `view.webContents.on('dom-ready')`): on every page load, `view.webContents.insertCSS(ADBLOCKER_COSMETIC_CSS)` injects a stylesheet that hides common ad-slot patterns (`.ad`, `.ads`, `.advertisement`, `.sponsored`, `.promoted`, `[data-ad]`, `[aria-label="advertisement"]`, etc.) with `display: none !important`. v1.3.2 also adds YouTube / YouTube Music selectors (`ytd-ad-slot-renderer`, `ytmusic-ad-slot-renderer`, `.ytp-ad-text-overlay`, `.ytp-ad-overlay-container`, `#player-ads`, `#masthead-ad`, `ytd-promoted-video-renderer`, etc.) so the Sponsored overlay, ad-slot renderers, and image-overlay banners are hidden on YouTube too. This catches DOM-embedded ad slots like Spotify's "Sponsored" row, which network blocking alone can't touch.
...
- Palette update: `Update Ad Blocker List` calls `updateAdList`, which fetches `https://pgl.yoyo.org/adservers/serverlist.php?hostformat=hosts` (HTTPS, 15s timeout, one redirect), parses it, replaces the in-memory Set, resets the counter, and persists to `userData/adblocker-list.txt`. Result comes back through the same `adblocker-update` event.
- **Note on the v1.3.0 engine attempt**: we briefly tried swapping this for `@ghostery/adblocker` (a WebAssembly port of the uBO filter engine). The `require('@ghostery/adblocker')` at the top of `main.js` crashed the main process on this machine's Electron build, so the app wouldn't even start. v1.3.1 reverted to the hand-rolled approach above, which is known to work.
